...
def run() raises -> Int:
    ...
    f += check(rv == 0, "socketpair created")
    var a = pair[0]
    var b = pair[1]
    ...
    f += check(len(events0) == 0, "no events when no data available")

    # Write through `a`. send() defaults to blocking.
    var msg = "epoll!".as_bytes()
    var nsent = sys_send(a, msg.unsafe_ptr(), len(msg), 0)
    f += check(nsent == 6, "send 6 bytes through socketpair[0]")

    # Now wait — should report `b` is readable within ~100 ms.
    var events1 = poller.wait(max_events=4, timeout_ms=500)
    f += check(len(events1) == 1, "exactly one event ready")
    var e = events1[0]
    f += check(e.fd == b, "the right fd fired")
    f += check(e.is_readable(), "fd is readable")
    ...
    f += check(got == 6, "recv returned 6")
    var as_string = String("")
    for i in range(6):
        as_string += chr(Int(rbuf[i]))
    f += check(as_string == "epoll!", "recv contents")
    var again = sys_recv(b, rbuf.unsafe_ptr(), 128, MSG_DONTWAIT)
    f += check(again == -11, "second recv would block (EAGAIN)")
    ...
    f += check(len(events2) == 1, "writable event fires")
    f += check(events2[0].is_writable(), "fd is writable")
...
def main() raises:
    var fails = run()
    if fails > 0:
        raise Error("test_poller: " + String(fails) + " failures")
