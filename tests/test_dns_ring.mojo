...
def _test_resolver() raises:
    # two A records
    var a = resolve_dns("a.test", server=_server())
    check(len(a) == 2, "dns: a.test record count")
    check(
    ...
    check(len(aaaa) == 1 and aaaa[0].is_v6, "dns: aaaa.test v6 answer")
    check(
    ...
    check(len(dropped) == 0, "dns: retry path reached the server")
    ...
    try:
        ...
        caught = True
    check(caught, "dns: nxdomain raises")
...
def _test_parser_negatives() raises:
    var q = dns_build_query(UInt16(0x1234), "a.test", QTYPE_A)
    check(len(q) == 12 + 8 + 4, "dns: query length")
    check(q[0] == 0x12 and q[1] == 0x34, "dns: query txid bytes")
    check(q[2] == 0x01, "dns: RD set")
